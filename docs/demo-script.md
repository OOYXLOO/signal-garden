...
8. Show the comment challenge, Reddit post draft, review snapshot, launch packet, and daily recap.
   - Say: "The comment prompt invites routes back in, the post draft prepares a user-approved demo post, and the launch packet gathers the demo-post setup in one copy."
   - Show the comment challenge with a Review link, the Reddit post draft, the review snapshot, the launch packet sections, then the recap text with route count and contributor lead.
